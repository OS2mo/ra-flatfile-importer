...
def gen_classes() -> Dict[str, List[Union[Tuple[str, str, str], str]]]:
    return {
...
def get_class_map(seed: str) -> Dict[str, UUID]:
    def gen_tuples() -> Iterator[Tuple[str, UUID]]:
        for facet_user_key, classes in gen_classes().items():
            for clazz in classes:
                if isinstance(clazz, tuple):
                    user_key, _, _ = clazz
                else:
                    user_key = clazz
                uuid = generate_uuid(seed + facet_user_key + user_key)
                yield user_key, uuid
    return dict(gen_tuples())
