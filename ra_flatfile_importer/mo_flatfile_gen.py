...
def generate_mo_flatfile(name: str, num_employees: int = 100):
    ...
    classes = [
        "PhoneEmployee", "EmailEmployee"
    ]
    class_map = dict(map(lambda user_key: (user_key, generate_uuid(seed + user_key)), classes))

    def gen_post_address():
        return (address.street_name() + " " + address.street_number() + ", " +
                address.postal_code()[3:] + " " + address.city())
    ...
    def create_employee_address(employee: Employee, class_uuid, generator_func) -> Address:
    ...
    create_phone_addresses = partial(create_employee_address, class_uuid=class_map["PhoneEmployee"], generator_func=partial(person.telephone, "########"))
    create_email_addresses = partial(create_employee_address, class_uuid=class_map["EmailEmployee"], generator_func=person.email)
